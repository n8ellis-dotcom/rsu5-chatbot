#!/usr/bin/env python3
"""
RSU5 Embed Script - CI version (reads env vars directly, no .env.local)
Runs on GitHub Actions with full RAM - no local memory issues.
"""
import os, sys, json
import psycopg2
...
if not DATABASE_URL:  print('❌ DATABASE_URL missing'); sys.exit(1)
if not OPENAI_API_KEY: print('❌ OPENAI_API_KEY missing'); sys.exit(1)
print('✅ Env vars loaded')

CHUNK_SIZE    = 800
CHUNK_OVERLAP = 100
DIRS          = ['docs', 'transcripts']
...
def get_all_files(directory):
    p = Path(directory)
    if not p.exists(): print(f'  ⚠️  Skipping {directory}'); return []
...
for filepath in all_files:
    try:
        ...
        if not chunks: files_done += 1; continue
...
cur.close()
conn.close()
print(f'\n🎉 Done! {total_chunks} chunks loaded. {errors} errors.')
